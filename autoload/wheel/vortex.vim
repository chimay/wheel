...
fun! wheel#vortex#add_torus (...)
	...
	if ! has_key(g:wheel.toruses, a:torus_name)
		echo "Adding torus" a:torus_name
		let g:wheel.current = a:torus_name
		let g:wheel.toruses[a:torus_name] = {}
	endif
endfu
...
fun! wheel#vortex#add_circle (circle_name)
	...
	if ! has_key(cur_torus, a:circle_name)
		echo "Adding circle" a:circle_name
		let g:wheel.toruses[torus_name].current = a:circle_name
		let g:wheel.toruses[torus_name].circles[a:circle_name] = {}
	endif
endfu

fun! wheel#vortex#add_location ()
	if empty(g:wheel)
		wheel#vortex#add_torus()
	endif
	let torus_name = g:wheel.current
	let cur_torus = g:wheel.toruses[torus_name]
	if empty(g:wheel.toruses[torus_name])
		wheel#vortex#add_circle()
	endif
	...
	if index(cur_circle.locations, here) < 0
		...
		let g:wheel.toruses[torus_name].circles[circle_name].current = a:circle_name
		let g:wheel.toruses[torus_name].circles[circle_name].circles[a:circle_name] = here
	endif
endfun
